var baidufile = {
  ...
  bindDate:function (mode,moduleID,uploadifyID){
	var moduleType ="";	
 	...
 	str = "";
     $.ajax({
         url: '/fxpgm/uploadfileAction_getFiles',
         type: 'POST',
         async: false,
         dataType : 'json',
         ...
         success: function (msg) { 
             ...
             for (var i = 0; i < objs.length; i++) {
                 ...
                 str = str + '<div style="position: relative; float: left; width:80px; height:100px; border:1px solid #E8E8E8; margin-left:30px;margin-top:0px;">' +
                             ...
                             '<a href="/fxpgm/uploadfileAction_downLoadFile?id='+objs[i].id+'" target="_blank">' +
...
bindAttDate:function (mode,moduleID,uploadifyID){
	...
	str = "";
  $.ajax({
      url: '/fxpgm/uploadfileAction_getAttFiles',
      type: 'POST',
      async: false,
      dataType : 'json',
      data: 'moduleType=' + moduleType + "&moduleID=" + moduleID,
...
GetIcon : function(id,fileType,url) {
	var contextPath = "/fxpgm";
    var exts = ["png", "jpeg", "jpg", "gif", "bmp"];
    for (var i = 0; i < exts.length; i++) {
        if (fileType.toLowerCase() == exts[i]) return "/fxpgm/uploadfileAction_viewImages?id="+id;//contextPath+"/images/file/img.png"; //return url;
    }
...
DelFile : function(fileID,mode,moduleID,uploadifyID){
	if(confirm("确实要删除吗？")){
    	$.ajax({
            url: '/fxpgm/uploadfileAction_delFile',
            type: 'POST',
            async: false,
            dataType : 'json',
            data: 'id=' + fileID,
...
getMutifiFile : function (moduleID){
	var str = '[]';
	$.ajax({
	         url: '/fxpgm/uploadfileAction_getFilesUrl',
	         type: 'POST',
	         async: false,
	         dataType : 'json',
