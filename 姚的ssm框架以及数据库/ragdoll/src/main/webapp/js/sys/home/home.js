...
var CenterTab = {
    ...
    update: function (id, title, iconCls, content, closable,url) {
    	 $("#"+this.Id).tabs('close',title);
        if ($("#" + this.Id).tabs('exists', title)) {
        	//var tab = $('#' + this.Id).tabs('getSelected');
        	var tab = $('#' + this.Id).tabs('select', title);
        	$("#" + this.Id).tabs('update', {
        		tab: tab,
        	    options: {
                title: title,
...
var Home = {
    ...
    */
    init: function () {
        ...
        this.addTab("0", "首页", "icon-tip", "index", "href",false);
    },
    ...
    */
    ...
    addTabAgain: function (id, title, iconCls, info, type,closable) {
    	if(info!=''&&info!='null'){
	        var content = '';
	        if (type == "href") {
	        	//content = '<div>'+title+'</div>';
	        ...
	        CenterTab.update(id, title, iconCls, content,closable,info);
    	}
    },
